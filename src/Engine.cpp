...
bool Engine::raycast_camera_occlusion(const glm::vec3 &start, glm::vec3 &end)
{
	glm::vec3 ray_dir = end - start;
	...
	for (float dist = 0.0f; dist < ray_len; dist += step_size)
	{
		glm::vec3 current_pos = start + ray_dir * dist;
		int blockX = static_cast<int>(floor(current_pos.x));
		int blockY = static_cast<int>(floor(current_pos.y));
		int blockZ = static_cast<int>(floor(current_pos.z));

		Block block = get_block(blockX, blockY, blockZ);
		if (BlockDatabase::get().get_block_data(block.id).is_solid)
		{
			end = current_pos - ray_dir * 0.1f;
			return true;
		}
	}
	return false;
...
void Engine::processInput(float dt, bool &mouse_enabled, double &lx, double &ly)
{
	...
	if (xNow && !m_key_X_last_state)
	{
		m_player_ptr->cycle_camera_mode();
	}
	m_key_X_last_state = xNow;

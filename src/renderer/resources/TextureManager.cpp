...
void TextureManager::createTextureImage(VkCommandBuffer cmd, std::vector<VmaBuffer> &stagingBuffers, const char *path)
{
    ...
    UploadHelpers::transitionImageLayout(
        cmd, m_TextureImage.get(), VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        range, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);

    UploadHelpers::copyBufferToImage(cmd, stagingBuffer.get(), m_TextureImage.get(), static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));

    UploadHelpers::transitionImageLayout(
        cmd, m_TextureImage.get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
